...
The player-facing **Transfer cargo** dialog builds on those rules with explicit source and destination endpoints. At a selected system it lists the friendly or uncolonized planetary surface and every co-located friendly fleet. The source is excluded from the destination list. A single atomic order may move Colonists, Ironium, Boranium and Germanium planet → fleet, fleet → planet or fleet → fleet. Sliders and exact numeric fields show the source remainder, destination result, shared hold usage and gross mass live. Each slider's right edge is the amount that fits in the destination hold after existing cargo and other selected transfers, capped by available source stock. These limits update together; unloading to a surface is limited only by source stock. Population may move only through a friendly colony, and loading from one always leaves at least one colonist behind. Minerals may be deposited on or collected from an uncolonized surface stockpile.
