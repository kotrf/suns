...
FleetTelemetry authoritative_snapshot(const Fleet& fleet, std::uint64_t observedTurn)
{
...
FleetTelemetry confirmed_fleet_telemetry(const GameState& state, const Fleet& fleet)
{
    ...
    return authoritative_snapshot(fleet, state.turn);
}
...
{
    for (auto& fleet : state.fleets) {
        const auto snapshot = authoritative_snapshot(fleet, observationTurn);
